...
# Muat file .env agar API Key terbaca
load_dotenv()
...
def buy():
    if request.method == "GET":
        ...
        # Validasi input
        if not symbol:
            return errorPage(title="No Data", info="Please enter a stock symbol", file="no-data.svg")
        ...
        try:
            ...
            return errorPage(title="Bad Request", info="Please enter a valid number of shares", file="animated-400.svg")
        ...
        # Dapatkan saldo pengguna saat ini
        available_cash = Users.query.filter_by(id=user).first().cash

        # Pastikan saldo mencukupi
        if available_cash < total_cost:
            return errorPage(title="Forbidden", info="Insufficient funds to complete transaction", file="animated-403.svg")

        # Kirim permintaan ke transaction-service untuk menyimpan transaksi
        response = requests.post("http://127.0.0.1:5002/buy", json={
            "user_id": user,
            "symbol": symbol,
            "shares": shares,
            "price": price
        ...
        # Jika transaction-service gagal, tampilkan pesan error
        if response.status_code != 201:
            return errorPage(title="Service Error", info="Failed to complete stock purchase", file="animated-500.svg")

        # Kurangi saldo pengguna di database monolitik
        Users.query.filter_by(id=user).update({"cash": available_cash - total_cost})
        db.session.commit()
...
def quote():
    if request.method == "GET":
        ...
        # Debugging: Print simbol saham yang dimasukkan
        print(f"Symbol entered: {symbol}")

        # Cek apakah simbol telah diisi
        if not symbol:
            return errorPage(title="No Data", info="Please enter a stock symbol", file="no-data.svg")

        try:
            # Memanggil microservice stock-quote-service
            response = requests.get(f"http://127.0.0.1:5001/quote?symbol={symbol}")

            # Debugging: Print status code dan response dari microservice
            print(f"Response Status Code: {response.status_code}")
            print(f"Response Text: {response.text}")

            # Jika microservice tidak merespons dengan kode 200, tampilkan error
            if response.status_code != 200:
                return errorPage(title="Service Unavailable", info="Stock quote service is unavailable", file="animated-503.svg")

            # Coba parse JSON
            try:
                data = response.json()
            except ValueError:
                return errorPage(title="Invalid Response", info="Invalid response from stock service", file="animated-500.svg")

            # Pastikan respons mengandung data yang valid
            if "price" not in data or "symbol" not in data:
                return errorPage(title="Bad Request", info="Please enter a valid stock symbol", file="animated-400.svg")

        except requests.exceptions.RequestException as e:
            print(f"Error fetching data from microservice: {e}")  # Debugging
            return errorPage(title="Service Error", info="Failed to connect to stock quote service", file="animated-503.svg")

        # Jika berhasil, tampilkan hasilnya di quoted.html
